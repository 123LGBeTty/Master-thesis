...
View(X02EN22_SP_Pia_better_noneg)
# load data
library(readxl)  
##all data, without negative values
EN22_SP_Pia <- read_excel("D:/Masterarbeit/Daten/02EN22_SP_Pia_better_noneg.xlsx") 
...
shapiro.test( EN22_SP_Pia$Nap) # signifikant
shapiro.test( EN22_SP_Pia$Acy) # signifikant
shapiro.test( EN22_SP_Pia$Ace) # signifikant
shapiro.test( EN22_SP_Pia$Fle) # signifikant
shapiro.test( EN22_SP_Pia$Phe) # signifikant
shapiro.test( EN22_SP_Pia$Ant) # signifikant
shapiro.test( EN22_SP_Pia$Fla) # signifikant
shapiro.test( EN22_SP_Pia$Py) # signifikant
shapiro.test( EN22_SP_Pia$BaA) # signifikant
shapiro.test( EN22_SP_Pia$Chr) # signifikant
shapiro.test( EN22_SP_Pia$BbF) # signifikant
shapiro.test( EN22_SP_Pia$BaP) # signifikant
shapiro.test( EN22_SP_Pia$Ipy) # signifikant
shapiro.test( EN22_SP_Pia$BPe) # signifikant
shapiro.test( EN22_SP_Pia$DahA) # signifikant
...
# PCA
## 1 and 2 dimension
EN22_SP_Pia2 <- read_excel("D:/Masterarbeit/Daten/02EN22_SP_Pia_better_noneg.xlsx") 
View(EN22_SP_Pia2)
#### only numerical data
...
fviz_pca_biplot(pca_result, label = "var",
                ...
                palette = c("darkgreen", "blue", "orange"),
                geom.ind = "point",
                pointshape = 19) +
...
ggplot(EN22_SP_Pia_saeulen, aes(x = combination, y=value, fill = PAH)) +
  ...
  scale_x_discrete(labels = function(x) {
    sapply(x, function(label) {
      # Hol den passenden Horizont für jedes Label
      horiz <- EN22_SP_Pia_saeulen$horizon[which(EN22_SP_Pia_saeulen$combination == label)[1]]
      # Färbe das Label nach dem Horizont ein
      paste0('<span style="color:', horizon_colors[horiz], ';">', label, '</span>')
    })
  }) +
  theme_minimal() +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_markdown(angle = 45, hjust = 1),  # Verwende element_markdown
        axis.ticks.x = element_blank(),
        text = element_text(family = "Calibri", size = 12),     # Schriftart und Schriftgröße allgemein
        axis.title = element_text(family = "Calibri", size = 14),  # Achsentitel
        axis.text = element_text(family = "Calibri", size = 10),   # Achsentext
        plot.title = element_text(family = "Calibri", size = 16, face = "bold"))  # Diagrammtitel
...
### also done with sum and ratio
# Notwendige Bibliotheken laden
library(ggplot2)
library(tidyr)
library(dplyr)

# Daten in Long-Format umwandeln und die Variablen in der gewünschten Reihenfolge als Faktor definieren
...
### with diagnostic ratios
# Facettierten Boxplot mit 2 Zeilen und 3 Spalten erstellen
